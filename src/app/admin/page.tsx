    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="rounded-lg border border-neutral-200 bg-white p-4">
        ...
        <form action={createSeason} className="flex flex-wrap items-end gap-3">
          ...
          <label className="flex flex-col text-sm">
            Qualifying opens
            <input name="qualification_start" type="date" className="rounded-md border border-neutral-300 px-2 py-1.5" />
          </label>
          <label className="flex flex-col text-sm">
            Qualifying closes
            <input name="qualification_end" type="date" className="rounded-md border border-neutral-300 px-2 py-1.5" />
          </label>
        ...
        <>
              ...
              {(["qualifying", "teams_set", "in_progress", "completed"] as const).map((status) => (
                <form key={status} action={setSeasonStatus}>
                  ...
                  <button
                    ...
                    className="rounded-md border border-neutral-300 px-3 py-1.5 text-sm disabled:cursor-not-allowed disabled:opacity-40 hover:border-green-700"
                  >
                    Set: {status}
                  </button>
                </form>
                    ...
                    <tr key={t.id} className="border-b border-neutral-100">
                      <td className="py-1.5 pr-2">
                        <form action={updateTeamPlayer} className="flex items-center gap-1">
                          ...
                          <select name="player_id" defaultValue={t.player_1_id} className="rounded border border-neutral-300 px-1 py-0.5">
                            {(profiles ?? []).map((p) => (
                      ...
                      <td className="py-1.5 pr-2">
                        <form action={updateTeamPlayer} className="flex items-center gap-1">
                          ...
                          <select name="player_id" defaultValue={t.player_2_id} className="rounded border border-neutral-300 px-1 py-0.5">
                            {(profiles ?? []).map((p) => (
                              <option key={p.id} value={p.id}>
                                {p.name}
                              </option>
                      ...
                      <td className="py-1.5 text-xs text-neutral-400">manual override</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <section className="rounded-lg border border-neutral-200 bg-white p-4">
            ...
            <form action={completeSeason} className="flex flex-wrap items-end gap-3">
              ...
              <label className="flex flex-col text-sm">
                Photo URL (optional)
                <input name="photo_url" type="url" placeholder="https://…" className="rounded-md border border-neutral-300 px-2 py-1.5" />
              </label>
          ...
          <tbody>
            {rosterWithHandicap.map(({ profile: p, handicap }) => (
              <tr key={p.id} className="border-b border-neutral-100">
                ...
                <td className="py-1.5 pr-2 text-neutral-500">{p.email}</td>
                ...
                <td className="py-1.5 pr-2">
                  <form action={updatePlayerRole} className="flex items-center gap-1">
                    ...
                    <select name="role" defaultValue={p.role} className="rounded border border-neutral-300 px-1 py-0.5">
